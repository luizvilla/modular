...
so widgets/tabs are pure frontend and privileged work stays in `main.js`.

## Renderer back-end touchpoints to remove

### Direct Node/Electron usage in renderer
- `dashboard/js/tabs.js`: `window.require('electron'|'fs'|'path'|'url')`
- `dashboard/js/dashboard_control.js`: `require('electron')`, `require('fs')`
- `dashboard/js/activity_toasts.js`: `window.require('electron')`
- `dashboard/examples/example_viewer.js`: `require('electron')`, `require('fs')`
- Many plugins/widgets use `window.require('electron').ipcRenderer`

### Renderer file system access
- `dashboard/js/tabs.js`: `fs.promises.readdir`, `fs.promises.readFile`
- `dashboard/js/dashboard_control.js`: `fs.promises.readFile`
- `dashboard/examples/example_viewer.js`: `fs.promises.readdir`, `fs.promises.readFile`
...
From `main.js` (inventory, 2026-02-04):
