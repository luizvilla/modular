...
| TUT-004 | `core/fast-frame-from-csv` | Fast Frame From CSV | planned | Copy repo-root [2026-05-22_17-01-17-record.csv](/home/luiz-villa/code/modular/2026-05-22_17-01-17-record.csv) into `app/extensions/tutorials/assets/sample_data.csv` and reuse it from there | Add tutorial `resources`, step `actions[]`, Fast Frame editor CSV-apply action, and `fast_frame_plot_configured` | Tutorial happy-path E2E, `tests/e2e/fast-frame.spec.js`, relevant integrated editor regression coverage, plus extension-runtime unit coverage if loader/schema code changes | `TBD` (`feat(tutorials): add fast-frame csv tutorial`) | Blank-dashboard Fast Frame tutorial using the shared `sample_data.csv` asset and one math channel. |
